...
export async function simulateTransactions({ network, txnBytes }) {
  const algod = getAlgod(network);
  const decoded = txnBytes.map((b) => new Uint8Array(Buffer.from(b, "base64")));

  const stxns = decoded.map((d) => {
    try {
      return algosdk.decodeSignedTransaction(d);
    } catch {
      const txn = algosdk.decodeUnsignedTransaction(d);
      const simBytes = algosdk.encodeUnsignedSimulateTransaction(txn);
      return algosdk.decodeSignedTransaction(simBytes);
    }
  });
...
function txnToJson(txn) {
  const encoded = algosdk.encodeUnsignedTransaction(txn);
  return algosdk.decodeObj(encoded);
}
